...
public class Developers {
    ...
    private static final int[] COLUN= {100, 385};
    private static final int[] LINE = {145, 405};
    ...
    public List<JLabel> getDeveloperLabels() {
        ...
        for (int i = 0; i < developerList.size(); i++) {
            ...
            ImageIcon icon = getIcon(dev.getImage(), dev.getName());
            label.setIcon(icon);
            label.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
            label.setBounds(LINE[line], COLUN[column], COMPONENTS_WIDTH, COMPONENTS_HEIGHT);
            label.setBorder(BorderFactory.createMatteBorder(1, 1, 1, 1, Color.BLACK));
            label.setCursor(new Cursor(Cursor.HAND_CURSOR));
            label.addMouseListener(mouseListenerGitHub(dev.getGithubUrl()));
            labels.add(label);
        }
        return labels;
    }
